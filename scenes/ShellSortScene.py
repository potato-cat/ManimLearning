...
class ShellSortScene(Scene):
    def construct(self):
        ...
        cardGroup1 = self.generateCards(0).next_to(self.title, DOWN, MED_LARGE_BUFF)
        step1 = Tex('原数组:',
                    tex_template=my_ctex(22), tex_environment='flushleft', font_size=28).next_to(cardGroup1, LEFT)
        self.play(Create(cardGroup1), Create(step1))
        self.array = self.arrays[1]
        cardGroup2 = self.generateCards(0).next_to(cardGroup1, DOWN)
        step2 = Tex('$h=7:$',
                    tex_template=my_ctex(22), tex_environment='flushleft', font_size=28).next_to(cardGroup2, LEFT)
        self.play(Create(cardGroup2), Create(step2))
        self.array = self.arrays[2]
        cardGroup3 = self.generateCards(0).next_to(cardGroup2, DOWN)
        step3 = Tex('$h=3:$',
                    tex_template=my_ctex(22), tex_environment='flushleft', font_size=28).next_to(cardGroup3, LEFT)
        self.play(Create(cardGroup3), Create(step3))
        self.array = self.arrays[3]
        cardGroup4 = self.generateCards(0).next_to(cardGroup3, DOWN)
        step4 = Tex('$h=1:$',
                    tex_template=my_ctex(22), tex_environment='flushleft', font_size=28).next_to(cardGroup4, LEFT)
        self.play(Create(cardGroup4), Create(step4))

        self.wait(2)

        self.play(Uncreate(cardGroup1), Uncreate(step1),
                  Uncreate(cardGroup2), Uncreate(step2),
                  Uncreate(cardGroup3), Uncreate(step3),
                  Uncreate(cardGroup4), Uncreate(step4))
    ...
    def sort(self):
        ...
        for m in range(len(hs)):
            h = hs[m]
            if self.step == None:
                self.step = Tex(r'$h=%d$' % h, tex_template=my_ctex(22), tex_environment='flushleft',
                                font_size=28).next_to(self.cardGroup, LEFT)
                self.play(Create(self.step))
            else:
                step = self.step
                self.step = Tex(r'$h=%d$' % h, tex_template=my_ctex(22), tex_environment='flushleft',
                                font_size=28).move_to(step, LEFT)
                self.play(ReplacementTransform(step, self.step))
            for i in range(0, h):
                if h > 1:
                    animations = []
                    for j in range(i, len(self.array), h):
                        animations.append(
                            self.cards[j].animate(run_time=RUN_TIME).shift([0, -CARD_MAX_HEIGHT - SMALL_BUFF, 0]))
                    self.play(*animations)
                for j in range(i + h, len(self.array), h):
                    card = self.cards[j]
                    self.play(card.animate(run_time=RUN_TIME).shift([0, -CARD_MAX_HEIGHT - SMALL_BUFF, 0]))
                    for k in range(j, h - 1, -h):
                        p = self.cards[k - h].get_critical_point(DOWN)
                        if self.pointer == None:
                            self.pointer = self.generatePointer().move_to(p - [0, 0.1, 0], UP)
                            self.play(Create(self.pointer, run_time=RUN_TIME))
                        elif k == j:
                            self.pointer.move_to(p - [0, 0.1, 0], UP)
                            self.wait(0.5)
                        else:
                            self.play(self.pointer.animate(run_time=RUN_TIME).move_to(p - [0, 0.1, 0], UP))
                        if self.array[k] < self.array[k - h]:
                            self.swap(k, k - h)
                        else:
                            break
                    self.play(card.animate(run_time=RUN_TIME).shift([0, CARD_MAX_HEIGHT + SMALL_BUFF, 0]))
                if h > 1:
                    ...
                    for j in range(i, len(self.array), h):
                        animations.append(
                            self.cards[j].animate(run_time=RUN_TIME).shift([0, CARD_MAX_HEIGHT + SMALL_BUFF, 0]))
                    self.play(*animations)
            self.arrays.append(np.copy(self.array))
